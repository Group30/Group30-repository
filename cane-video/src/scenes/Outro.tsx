  ...
  return (
    <AbsoluteFill style={{ opacity: op, fontFamily: theme.font, color: theme.ink }}>
      <div style={{ position: "absolute", left: 0, right: 0, top: height * 0.2, opacity: listOut }}>
        <div style={{ textAlign: "center", fontFamily: theme.mono, fontSize: 19, letterSpacing: 4, color: theme.blue, marginBottom: 30 }}>앱 기능</div>
      ...
      <div style={{ position: "absolute", left: 0, right: 0, top: height * 0.32, textAlign: "center", opacity: titleIn, transform: `scale(${0.92 + 0.08 * titleIn})` }}>
        ...
        <div style={{ fontSize: 34, fontWeight: 800 }}>{product.team} · {product.org}</div>
        <div style={{ fontFamily: theme.mono, fontSize: 19, letterSpacing: 3, color: theme.grey, marginTop: 10 }}>{product.contest}</div>
      </div>
      <div style={{ position: "absolute", right: 160, bottom: 200, width: 130, height: 130, border: `2px dashed ${theme.line}`, borderRadius: 12, display: "flex", alignItems: "center", justifyContent: "center", fontFamily: theme.mono, fontSize: 13, color: theme.grey, textAlign: "center", opacity: titleIn }}>
        QR /<br />로고 자리
      </div>
      <div style={{ position: "absolute", left: 0, right: 0, bottom: 150, textAlign: "center", fontFamily: theme.mono, fontSize: 14, letterSpacing: 1, color: theme.grey, opacity: titleIn }}>
        {product.musicCredit}
      </div>
